...
export class intraAuth1645622620898 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` CHANGE \`oauth_token\` \`github_uid\` varchar(42) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`github_username\` varchar(20) NOT NULL AFTER \`character\``,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` CHANGE \`github_uid\` \`oauth_token\` varchar(42) NOT NULL`,
    );
